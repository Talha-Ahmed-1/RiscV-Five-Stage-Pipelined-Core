...
object Launcher {
  val examples = Map(
	...
	"BranchForwardUnit" -> { (manager: TesterOptionsManager) =>
        Driver.execute(() => new BranchForwardUnit(), manager) {
          (c) => new BranchForwardUnitTests(c)
        }
      },
	"StallingUnit" -> { (manager: TesterOptionsManager) =>
        Driver.execute(() => new StallingUnit(), manager) {
          (c) => new StallingUnitTests(c)
        }
      },
	...
	"ForwardingUnit" -> { (manager: TesterOptionsManager) =>
        Driver.execute(() => new ForwardingUnit(), manager) {
          (c) => new ForwardingUnitTests(c)
        }
      },
